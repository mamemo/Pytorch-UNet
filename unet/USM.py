...
class LoG2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, fixed_coeff=False, sigma=-1, stride=1, padding=0, dilation=1, cuda=True):
        ...
        if not self.fixed_coeff:
            if self.cuda:
                self.sigma = nn.Parameter(torch.cuda.FloatTensor(1), requires_grad=True)
            else:
    ...
    def forward(self, input):
        ...
        kernel = kernel.repeat(self.out_channels, self.in_channels, 1).view(self.out_channels, self.in_channels, self.kernel_size, -1)
        res = F.conv2d(input, -kernel, padding=self.padding)#, stride=self.stride, padding=self.padding, dilation=self.dilation)
        return res
...
class USMBase(LoG2d):
    ...
    def __init__(self, in_channels, kernel_size, fixed_coeff=False, sigma=-1, stride=1, dilation=1, cuda=True, batch_len = 10):
        ...
        super(USMBase, self).__init__(in_channels, in_channels, kernel_size, fixed_coeff, sigma, stride, padding, dilation, cuda)
        self.alpha = None
    ...
    def forward(self, input):
        if self.cont == self.batch_len:
            print('Lambda del USM aprendido: ' + str(self.alpha.item()))
            self.cont = 0
        else:
            self.cont+=1
        
        input = (255*(input.double()/torch.max(input).double())).float()
...
class USM(USMBase):
    def __init__(self, in_channels, kernel_size, fixed_coeff=False, sigma=-1, stride=1, dilation=1, cuda=True, batch_len = 10):
        super(USM, self).__init__(in_channels, kernel_size, fixed_coeff, sigma, stride, dilation, cuda, batch_len)
        if self.cuda:
            self.alpha = nn.Parameter(torch.cuda.FloatTensor(1), requires_grad=True)
        else:
...
class AdaptiveUSM(USMBase):
    def __init__(self, in_channels, in_side, kernel_size, fixed_coeff=False, sigma=-1, stride=1, dilation=1, cuda=True):
        super(AdaptiveUSM, self).__init__(in_channels, kernel_size, fixed_coeff, sigma, stride, dilation, cuda)
        if self.cuda:
            self.alpha = nn.Parameter(torch.cuda.FloatTensor(in_side, in_side), requires_grad=True)
        else:
            self.alpha = nn.Parameter(torch.FloatTensor(in_side, in_side), requires_grad=True)
        self.i_weights()
